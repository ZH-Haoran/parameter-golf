...
**Ignore**: `autoresearch/` directory (reference only), `train_gpt_mlx.py` file (implementation on other platforms)
...
2. **Read**: `README.md`, `train_gpt.py` (first ~200 lines to understand hyperparameters)
...
6. **Commit**: `git add train_gpt.py results.tsv .gitignore && git commit -m "Stage 1: initial baseline"`
...
- `head -100 train_gpt.py` — current config
